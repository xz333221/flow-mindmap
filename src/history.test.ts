...
describe('useHistory', () => {
  ...
  it('after two records you can undo once', () => {
    ...
    expect(restored?.children[0].text).toBe('A')
    // one entry left
    expect(h.canUndo()).toBe(false)
  ...
  it('undo then redo re-applies the change', () => {
    ...
    h.record(tree)
    expect(h.undo()?.children[0].text).toBe('A')
    expect(h.redo()?.children[0].text).toBe('A-changed')
  })
  ...
  it('multiple undos walk back through the timeline', () => {
    const h = useHistory()
    h.record(tree) // captures initial state (text='A')
    tree.children[0].text = 'v1'
    h.record(tree) // v1
    tree.children[0].text = 'v2'
    h.record(tree) // v2
    tree.children[0].text = 'v3'
    h.record(tree) // v3 — current
    expect(h.undo()?.children[0].text).toBe('v2')
    expect(h.undo()?.children[0].text).toBe('v1')
    expect(h.undo()?.children[0].text).toBe('A')
